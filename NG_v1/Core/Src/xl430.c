...
struct prsRxData xl430_readbuffer()
{
	...
	_rxData.dataSize  = MAX_DATA_LENGTH - __HAL_DMA_GET_COUNTER(&hdma_usart6_rx);
...
{
	uint8_t _IDs[] = {0x01,0x02};  //Id 1 =r_motor,2-l_motor
	int speeds[] = {r_Speed,l_Speed};
	xl430_syncWrite(&GOAL_VELOCITY, _IDs, 2,speeds, 4);
	HAL_Delay(50);
}
